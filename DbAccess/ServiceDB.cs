...
namespace DbAccess
{
    public class ServiceDB
    {
        ...
        public static string GetInsertRowSql(DataRow dr)
        {
            ...
            for (int i = 0; i < fieldNo; i++)
            {
                ...
                else if (dt.Columns[i].DataType == typeof(Int32) || dt.Columns[i].DataType == typeof(Double) || dt.Columns[i].DataType == typeof(System.SByte))
                {
        ...
        public static int InsertRow(DataTable dt )
        {
            ...
            {
                listSql.Add(GetInsertRowSql(dt.Rows[i] ));
            }
        ...
        public static int DeleteRow(DataTable dt, string idColName)
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                listSql.Add( GetDeleteRowSql(dt.Rows[i], idColName));
            }
            return ExecuteNonSql(listSql);
             
        }

        public static string GetDeleteRowSql(DataRow dr, string idColName)
        {
        ...
        public static int UpdateRow(DataTable dt, string idColName)
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                listSql.Add(GetUpdateRowSql(dt.Rows[i], idColName));
            }
            return ExecuteNonSql(listSql);

        }
        public static string GetUpdateRowSql(DataRow dr, String idColName)
        {
            ...
            foreach (DataColumn mcs in dr.Table.Columns)
            {
                ...
                else if (mcs.DataType == typeof(Int32) ||mcs.DataType == typeof(Double) || mcs.DataType == typeof(System.SByte))
                {
                ...
                else if (mcs.DataType == typeof(DateTime))
                {
                    if (dr[mcs.ColumnName] == null || string.IsNullOrEmpty(dr[mcs.ColumnName].ToString()))
                    {
